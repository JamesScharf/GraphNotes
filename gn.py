...
class Node:
    ...
    def save(self):
        filehandler = open(h + "/GraphNotes/" + self.name + ".nd", 'wb')
...
def loadNodes():
    filenames = os.listdir(h + "/GraphNotes/")
    for f in filenames:
        if ".nd" in f:
            temp = pickle.load(open(h + "/GraphNotes/" + f, "rb"))
...
def createNewNote(name):
    ...
    if not name.strip():
        print(colored("Invalid whitespace in note name.", 'red'))
        return None
    print(colored("Note created. Opening external editor...", 'blue'))
    nodes[name] = Node(name)
    print("Type 'list' to see available connections.")
    ...
    for a in attempted_connections:
        if a in nodes:
            ...
            if not name.strip():
                print(a + " does not exist, creating note now...")
                createNewNote(a)
                nodes[name].addConnection(a)
...
def removeNote(name):
    ...
    #remove pickle file
    if os.path.exists(h + "/GraphNotes/" + name + ".nd"):
        os.remove(h + "/GraphNotes/" + name + ".nd")

    #Clean all connections
    for key in nodes:
        currNode = nodes[key]
        if currNode.hasConnection(name):
            currNode.removeConnection(name)
...
def graphToDot():
    ...
    result = result + "\n}"
    #write to file
    f = open(h + "/GraphNotes/graphView.dot", "w")
    f.write(result)
    return result

def viewDot():
    print(colored("\nGraph exportable from ", "white"), colored("~/GraphNotes/graphView.dot\n", 'magenta'))
    bashCommand = "graph-easy " + h + "/GraphNotes/graphView.dot"
    process = subprocess.Popen(bashCommand.split())
    #This prevents any overlap/drawing issues
    time.sleep(0.2)
...
def startup():
    ...
    print(colored("\nHere are your options:", 'cyan'))
    print("   a) Create new note")
    print("   b) List nodes")
    print("   c) Remove note")
    print("   d) Add new connection between notes")
    print("   e) Remove connection between notes")
    print("   f) Edit your settings file [not implemented]")
    print("   g) Visualize notes -- must have graphviz and Graph::Easy installed!")
...
if (len(sys.argv) == 2):
    ...
    if sys.argv[1] == 'graph' or sys.argv[1] == '-g':
        graphToDot()
        viewDot()

if (len(sys.argv) == 3):
    #delete note
    if (sys.argv[1] == 'del' or sys.argv[1] == '-d'):
        removeNote(sys.argv[2])


    if sys.argv[1] == "view" or sys.argv[1] == "-v":
        displayNote(sys.argv[2])

    if sys.argv[1] == "edit" or sys.argv[1] == "-e":
        if sys.argv[2] in nodes:
            #edit, don't make another
            nodes[sys.argv[2]].edit()
        else:
            createNewNote(sys.argv[2])